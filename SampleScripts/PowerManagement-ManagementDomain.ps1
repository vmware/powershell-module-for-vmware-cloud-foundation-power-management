...
if ($PsBoundParameters.ContainsKey("shutdown") -or $PsBoundParameters.ContainsKey("genjson")) {
    Try {
        ...
        if ($accessToken) {
                                    ...
                                    if (Test-vSphereAuthentication -server $vcfVcenterDetails.fqdn -user $vcfVcenterDetails.ssoAdmin -pass $vcfVcenterDetails.ssoAdminPass) {
                                        ...
                                        while ($counter -lt 1800) {
                                            ...
                                            foreach ($esxiNode in $esxiHosts) {
                                                if (Test-ManagementEndpoint -server $esxiNode -port 443) {
                                            ...
                                            if ($successCount -eq $esxiHosts.count) {
            ...
            # Gather vCenter Server Details and Credentials (VCF 5.0+ uses ssoId scoping).
            ...
            if (-Not (Test-ManagementEndpoint -server $vcServer.fqdn -Port 443) ) {
        ...
        foreach ($esxiNode in $esxiWorkloadDomain) {
            ...
            if ($runningVMs.count) {
                Write-LogMessage -Type WARNING -Message "Some VMs are still in powered-on state."
                Write-LogMessage -Type WARNING -Message "Cannot proceed until the powered-on VMs are shut down. Shut them down manually and run the script again."
            ...
            while ($counter -lt $maxSecondsToWait) {
                ...
                foreach ($esxiNode in $esxiWorkloadDomain) {
                    if (Test-ManagementEndpoint -server $esxiNode.fqdn -Port 443) {
                        Write-LogMessage -Type WARNING -Message "Some hosts are still up. '$($esxiNode.fqdn)' responded. Sleeping for 60 seconds before next check..."
                        break
                    } else {
                        $successCount++
                    }
...
if ($PsBoundParameters.ContainsKey("startup")) {
    Try {
        ...
        if (-Not (Test-ManagementEndpoint -server $vcServer.fqdn -Port 443 -WarningAction SilentlyContinue)) {
            Write-LogMessage -Type INFO -Message "Could not connect to $($vcServer.fqdn). Starting vSAN..."
            Write-Host ""
            $proceed = Read-Host "Please start all the ESXi hosts belonging to the cluster '$($cluster.name)' and wait for the host console to come up. Once done, please enter yes."
            if (-Not $proceed) {
                Write-LogMessage -Type WARNING -Message "None of the options is selected. Default is 'No', hence stopping script execution..."
                Exit
            } else {
        ...
        # Lockdown mode check, then restart cluster via wizard.
        Test-LockdownMode -server $vcServer.fqdn -user $vcUser -pass $vcPass -cluster $cluster.name
            ...
            if ($mgmtClusterIds.Count -ge 2) {
                foreach ($clusterId in $mgmtClusterIds) {
                    ...
                    if (!$isDefault) {
                        ...
                        $esxiHosts = (Get-VCFHost | Select-Object fqdn -ExpandProperty cluster | Where-Object { $_.id -eq $clusterId.id }).fqdn
                        foreach ($esxiNode in $esxiHosts) {
                            if (-Not (Test-ManagementEndpoint -server $esxiNode -port 443)) {
                                Write-LogMessage -Type WARNING -Message "ESXi host $esxiNode is not powered on."
                            }
                        }
                        # Lockdown mode check, then restart cluster via vSAN wizard.
                        Test-LockdownMode -server $vcUser -user $vcUser -pass $vcPass -cluster $clusterName
                        Set-VsanClusterPowerStatus -server $vcUser -user $vcUser -pass $vcPass -cluster $clusterName -PowerStatus clusterPoweredOn
                        # Check vSAN Status
                        if ( (Test-VsanHealth -cluster $clusterName -server $vcServer.fqdn -user $vcUser -pass $vcPass) -ne 0) {
                        ...
                        if ( (Test-VsanObjectResync -cluster $clusterName -server $vcServer.fqdn -user $vcUser -pass $vcPass) -ne 0) {
                            Write-LogMessage -Type ERROR -Message "vSAN object resynchronization failed. Check your environment and run the script again."
                            Exit
                        }
                        # Startup the vSphere Cluster Services Virtual Machines in the Management Workload Domain.
                        Set-Retreatmode -server $vcServer.fqdn -user $vcUser -pass $vcPass -cluster $clusterName -mode disable
                        $counter = 0
                        $retries = 10
                        $sleepTime = 30
                        while ($counter -ne $retries) {
